...
for (j in 1:length(flist)) {
  ...
  tower_data <- layer.isotope(flist[j], site, which_part = "H2")
  binded_data <- do.call(rbind, tower_data) 
	  # fit the model using all heights
  keelings <- binded_data %>% group_by(dom) %>% 
	    do(model = lm(I(H2_isoH2o*H2_rtioMoleWetH2o) ~ H2_rtioMoleWetH2o, data=.))
...
# write.csv(x = output.all.height, file = "C:/Users/libon/Box/neon_extrac_data/Scientific Data Code/keeling isotopes/et_H2_iso.csv")
